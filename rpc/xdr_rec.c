...
typedef struct rec_strm {
	 ...
	 * Is this non-blocked?
	 */
	u_int in_nonblock;	/* non-blocked input */
	u_int in_needpoll;	/* need to poll to get more data ? */
...
XDR_API bool_t		__xdrrec_getbytes_nonblock(XDR *, enum xprt_stat *);
...
#else /* _MSC_VER */
	pos = _lseek((intptr_t)rstrm->tcp_handle, 0, 1);
#endif /* _MSC_VER */
	if (pos != -1)
		switch (xdrs->x_op) {
...
flush_out(RECSTREAM *rstrm, bool_t eor)
{
	...
	*(rstrm->frag_header) = xdr_hton32(len | eormask);
...
fill_input_buf(RECSTREAM *rstrm, bool_t do_align)
{
	...
	if (do_align) {
		...
		u_int i = (uintptr_t)rstrm->in_boundry % BYTES_PER_XDR_UNIT;
...
__xdrrec_getbytes_nonblock(XDR *xdrs, enum xprt_stat *pstat)
{
	...
	while (len_received > 0) {
		if (rstrm->fbtbc == 0) {
			...
			last_frag = (xdr_ntoh32(minfraglen) & LAST_FRAG) != 0;
			minfraglen = xdr_ntoh32(minfraglen) & (~LAST_FRAG);
			/*
			 * The minimum record size we will need is whatever's
