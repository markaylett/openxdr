...
final class VarArrayCodec<T> implements Codec<ArraySlice<T>> {
    private final Codec<T> codec;
    private final int maxsize;

    VarArrayCodec(Codec<T> codec, int maxsize) {
        this.codec = codec;
        this.maxsize = maxsize;
    }

    VarArrayCodec(Codec<T> codec) {
        this(codec, Integer.MAX_VALUE);
    }

    static <T> void encodeVarArray(ByteBuffer buf, T[] val, int offset,
    ...
    static <T> void encodeVarArray(ByteBuffer buf, T[] val, Codec<T> codec,
    ...
    static <T> T[] decodeVarArray(ByteBuffer buf, Codec<T> codec, int maxsize)
            throws CharacterCodingException {
    ...
    static <T> T[] decodeVarArray(ByteBuffer buf, Codec<T> codec)
            throws CharacterCodingException {
